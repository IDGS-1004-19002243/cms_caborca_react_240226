              ...
              {contenido.carousel.map((_, index) => (
                <button
                  ...
                  className={`h-3 rounded-full transition-all ${
                    index === slideActual ? 'w-12 bg-white' : 'w-3 bg-white/50'
                  }`}
                />
              ))}
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center max-w-7xl mx-auto">
              ...
              <div className="order-2 md:order-1 relative group">
                ...
                {/* Badge años (ajustado) */}
                <div className="absolute -bottom-6 -right-6 hidden md:block" style={{ borderRadius: '9999px', backgroundColor: '#9B8674', color: '#fff', padding: '10px 12px', boxShadow: '8px 16px 36px rgba(0,0,0,0.18)' }}>
                  <div className="text-center">
                    <p className="text-2xl font-extrabold leading-none">{contenido.arteCreacion.anosExperiencia}+</p>
                    <p className="text-[10px] font-semibold tracking-wider uppercase">AÑOS</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col justify-center order-1 md:order-2">
                <div className="mb-4">
                  <span className="inline-block bg-caborca-cafe text-white text-xs font-bold tracking-widest px-4 py-2 rounded-full">{contenido.arteCreacion.badge[idioma]}</span>
                </div>
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif mb-6 text-caborca-cafe leading-tight">
                  {contenido.arteCreacion.titulo[idioma]}
                </h2>
                <div className="space-y-4 mb-8">
                  {contenido.arteCreacion.features.map((feature, i) => (
                    <div key={feature.id} className="flex items-start gap-3">
                      <div className={`shrink-0 w-10 h-10 ${i === 1 ? 'bg-gray-600' : 'bg-caborca-cafe'} rounded-full flex items-center justify-center mt-1`}>
                        <span className="text-white text-sm">✓</span>
                      </div>
                      <div>
                        <h4 className="font-bold text-caborca-cafe mb-1">{feature.titulo[idioma]}</h4>
                        <p className="text-caborca-cafe text-sm leading-relaxed">{feature.descripcion[idioma]}</p>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="flex items-center gap-4">
                  <a href="#" className="inline-flex items-center gap-2 bg-caborca-cafe text-white font-bold tracking-widest text-xs sm:text-sm px-6 sm:px-8 py-3 rounded-lg shadow-lg group">
                  ...
                  <div className="flex items-center gap-2 text-caborca-cafe text-sm">
                    ...
                    <span className="font-semibold">{contenido.arteCreacion.nota[idioma]}</span>
                  </div>
                </div>
              </div>
            </div>
